		...
		<nav class="navbar navbar-default navbar-fixed-top navbar-inverse">
			<div class="container">
				...
				<div class="navbar-header">
					...
					<a class="navbar-brand" href="#">教学平台</a>
				</div>
		...
		<div id="wrapper">
			...
			<nav id="nav">
				<c:choose>
					<c:when test="${user.userType.equals('老师')}">
						...
						<a href="#courseware" class="icon fa-file " id="coursewarePanel"><span>Courseware</span></a>
					</c:when>
					<c:otherwise>
						<a href="#homework" class="icon fa-book active" id="homeworkPanel"><span>Homework</span></a>
						<a href="#courseware2" class="icon fa-file " id="courseware2Panel"><span>Courseware</span></a>
					</c:otherwise>
				</c:choose>
				...
				<c:choose>
					<c:when test="${user.userType.equals('老师')}">
								...
								<table>
									...
									<c:forEach items="${homeworkList}" var="item">
										<tr>
											<td>
												<a id="showhomework" data-toggle="modal" data-target="#homeworkModal${item.homeworkId}">${item.homeworkName}</a>
											</td>
											<td>
							...
							<a href="#courseware" class="jumplink">
								<span class="icon fa-chevron-right"><span>See my Courseware</span></span>
							</a>
						</article>

						<%--课件start--%>
						<!-- Courseware -->
						<article id="courseware" class="panel">
							<header>
								<div class="titlebg">
									<h2>课件</h2>
								</div>
							...
							<div>
								<table>
									...
									<%int num = 0; %>
									<c:forEach items="${coursewareList}" var="item">
											<%num++; %>
											<tr>
												<td><%=num %></td>
												<td>${item.coursewareTitle}</td>
												<td>
													<fmt:formatDate value="${item.publicTime}" type="date" pattern="yyyy-MM-dd hh:mm:ss" />
												</td>
												<td>
													<a href="${item.src}" id="download" >下载</a>
													<a href="deleteCourseware?coursewareID=${item.coursewareID}" id="delete" >删除</a>
												</td>
											</tr>
									</c:forEach>
								</table>
								<div>
									<a data-toggle="modal" data-target="#postCourseware" href="#" onclick="return false;" class="btn btn-primary pull-right">上传课件</a>
								</div>
							</div>
							<div class="clearfix"></div>
					...
					<c:otherwise>
						...
						<article id="homework" class="panel">
							...
							<div>
								<table class="table">
									<tr>
										...
										<c:forEach items="${studentHomework}" var="shItem">
												<tr>
													<td>
														<a id="showhomework" data-toggle="modal" data-target="#homeworkModal${shItem.studentHomeworkRelationId}">${shItem.homework.homeworkName}</a>
													</td>
							...
							<a href="#courseware2" class="jumplink">
								<span class="icon fa-chevron-right"><span>See my courseware</span></span>
							</a>
						</article>
						<%--课件start--%>
						<!-- Courseware -->
						<article id="courseware2" class="panel">
							<header>
								<div class="titlebg">
									<h2>课件</h2>
								</div>
							...
							<div>
								<form method="post" action="LessonDetailSer?method=sendMessage">
									<div>
										...
										<input type="hidden" id="comment-message-lessonId" name="lessonId" value="${lessonIdd}">
								...
								<c:forEach items="${studentHomeworkRelationsList}" var="shrItem">
									<c:if test="${shrItem.homework.homeworkId==item.homeworkId}">
										...
										<tr>
											...
											<td>
												<a href="${shrItem.src}" id="download" >下载</a>
												<c:choose>
			...
			<c:forEach items="${studentHomeworkList}" var="shItem">
				...
				<div class="homeworkmodal">
					<div class="modal fade" id="homeworkModal${shItem.studentHomeworkRelationId}" tabindex="-1" role="dialog" aria-labelledby="homeworkModalLabel" aria-hidden="true">
			...
			<%--上传课件start--%>
			<div class="modal fade" id="postCourseware" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
				<div class="modal-dialog" role="document">
					<div class="modal-content">
						<div class="modal-header">
							<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
							<h4 class="modal-title" id="myModalLabel">上传课件</h4>
						</div>
